...
fn generate_uri() -> String {
    ...
    for arg in env::args().skip(1) {        //takes envarg in slot 1. slot 0 is the path to the executable itself. If you
                                            //pass it multiple envargs, it'll cycle through - useful if you have an 
                                            //aftermarket clipboard. Someone should make sure that works.
        let mut uri_string = &arg;
        output_uri = uri_string.replace("\"", "/");
        output_uri = output_uri.replace(" ", "%20");  //these three lines transform something like 
                                                      //\\server\the share\the program\program.exe to 
                                                      //file://server/the%20share/the%20program/program.exe which works better for this application.
